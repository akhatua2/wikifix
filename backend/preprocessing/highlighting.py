"""
Core highlighting functionality extracted from api/wikipedia.py for reuse in preprocessing.
This module contains all the text matching and HTML highlighting logic.
"""
...
def get_highlighting_css_and_js() -> str:
    """
    Get the CSS and JavaScript needed for highlighting functionality.
    """
    return """
<style>
...
def add_highlight_to_html(html_content: str, text_to_highlight: str, context_text: str = None, page_name: str = "") -> str:
    """
    Complete highlighting pipeline: fix HTML, find text match, highlight, and add CSS/JS.
    This is the main function for preprocessing.
    """
    if not text_to_highlight:
        return html_content
    
    print(f"Processing highlighting for page: {page_name}")
    print(f"Target text: {text_to_highlight[:100]}...")
    
    # Step 1: Fix HTML content for local serving
    fixed_content = fix_html_content(html_content, page_name)
    
    # Step 2: Find the best matching text
    actual_text_match = find_best_text_match(fixed_content, text_to_highlight, context_text)
    ...
    if actual_text_match:
        ...
        # Step 3: Highlight the text in HTML
        fixed_content = highlight_text_in_html(fixed_content, text_to_highlight, match_type)
    else:
        print(f"No match found for text: {text_to_highlight[:100]}...")
    
    # Step 4: Add CSS and JavaScript
    css_and_script = get_highlighting_css_and_js()
    
    # Insert CSS and script in the head
    if '<head>' in fixed_content:
        fixed_content = fixed_content.replace('<head>', f'<head>{css_and_script}', 1)
    elif '<html>' in fixed_content:
        fixed_content = fixed_content.replace('<html>', f'<html><head>{css_and_script}</head>', 1)
    else:
        fixed_content = css_and_script + fixed_content
    
    print(f"Highlighting processing completed for page: {page_name}")
